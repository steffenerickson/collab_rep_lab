//-----------------------------------------------------------------------------//
// Name:   Correspondence Test Function 
...
program define row_to_col, rclass
	...
	mata:row_to_col(tau)				 // mata row to column function
	tempname T
	matrix `T' = r(Vec)
...
program name_rows, nclass 
	args tau ref 
	confirm matrix `tau' 
	row_to_col `tau'					 // ensures tau is a colvector
	matrix tau = r(tau)
	local cn = rowsof(tau)
	forvalues x = 1/`cn' {				 // expand list 
		if `x' == 1 {
			local nums `x'
		}
		else {
	...
	local remove `ref'					 // removes reference for second list
	local nums2: list nums - remove

	// Treatment Effect Table			 // creates local macro with row names
	global names 
...
program correspondence, rclass 
	...
	name_rows `tau' `ref' // get row names to display results 
	...
	tempname effect diff equiv corr corr2
	// Significance Tests
	...
	// matrix `corr2' = r(corr2)
	// return matrix C2 = `corr2', copy
...
// Colvector helper function 
function is_colvec(z) return(anyof(("colvector","scalar"),orgtype(z)))

// Transpose to colvector help function 
...
struct inputs 
{
	...
	real scalar e_th 					// std treshold for equiv test 
							     ...
							     real scalar ref )
{
	...
	real scalar 				 i , a , b 
	row_to_col(tau) // ensures vector is column vector 
	row_to_col(se0) 
	//----------------------- Input Variables ----------
	...
	in.c = rows(tau)
	in.tau1 = J(1,in.c-1,tau[ref,1])' // reference tau Vector 
	in.se1 = J(1,in.c-1,se0[ref,1])'	 // reference se vector 
	in.tau2 = J(0,1,.) // comparison tau vector 
	in.se2 = J(0,1,.) // comparison se vector 
	for (i=1;i<=in.c; i++) {
		if (i == ref) continue 
		else {
			a = tau[i,1]
	...
	in.se3 = sqrt(in.se1:^2 + in.se2:^2)  // pooled se 
	in.delta = abs(in.tau1 - in.tau2) // difference 
...
struct effect_outputs scalar effect_stat(struct inputs scalar x)
{
	...
	for (i=1;i<=length(z); i++) {	
		a = (1 - normal(z[i,1])) // get p value 
		p = (p\a)
	}
	r.sig = p :<= x.alpha 
	
	// ci for individual studies 
	u_tau = x.tau +  x.z_crit:*x.se0 // Upper CI 
	l_tau = x.tau + -x.z_crit:*x.se0 // Lower CI 
	ci_tau = (l_tau,u_tau)
...
struct diff_outputs scalar diff_test(struct inputs scalar x)
{
	...
	x.se3 = sqrt(x.se1:^2 + x.se2:^2) // pooled standard error 
	x.delta = abs(x.tau1 - x.tau2) // differences 
	z_delta = x.delta :/ x.se3 // z score for differences 
	p_delta = J(0,1,.)
	
	for (i=1;i<=length(z_delta); i++) {	
		a = (1 - normal(abs(z_delta[i,1]))) // p-value for differences 
		p_delta = (p_delta\a)
	}
	
	r.sig_delta = p_delta :<= x.alpha / 2 // check for significance
	
	// ci for the difference 
	u_delta  = x.delta + x.z_crit:*x.se3 // upper CI 
	l_delta = x.delta + -x.z_crit:*x.se3 // lower CI 
...
struct equiv_outputs scalar equiv_test(struct inputs scalar x)
{
	...
	x.se3 = sqrt(x.se1:^2 + x.se2:^2)  // pooled se 
	x.delta = abs(x.tau1 - x.tau2) // difference 
	l_z_eq = (x.delta :+ -x.e_th):/x.se3 // critical value below e_th 
	u_z_eq  = (x.delta :+ x.e_th):/x.se3 // critical value above e_th 
	
	// pval below e_th
	p_eqp = J(0,1,.)
	for (i=1;i<=length(l_z_eq); i++) {	
		a = (normal(l_z_eq[i,1])) // p value lower tail 
		p_eqp = (p_eqp\a)
	}
	...
	for (i=1;i<=length(u_z_eq); i++) {	
		a = ((1 - normal(u_z_eq[i,1]))) // p value upper tail 
		p_eqn = (p_eqn\a)
	}
...
// use void function to return results to stata 
