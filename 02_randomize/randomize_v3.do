...
*	limit(real, default is 0) -Can specify max number of treatment allocations for 
*	each treatment arm.
...
if `limit' !=1 {	
	if `arms' == 2 {
		...
		local b = (`arms' * `limit' ) - `limit' // if arms is bigger than 2
	}						// , multiply the limit by the number of treatment arms because observations less than b will be equally split between the # of arms - subtract the limit to get an equal number of treatments in each arm 
...
*------------------------------------------------------------------------------*
* Appending data back together i
*------------------------------------------------------------------------------*
